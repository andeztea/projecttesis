...
class AbsenModel extends CI_Model
{
    ...
    protected $query;
    protected $column_order = array(null, 'b.thn_akademik', 'a.kategori', 'a.skor_nilai'); //set column field database for datatable orderable
    protected $column_search = array('b.thn_akademik', 'a.kategori', 'a.skor_nilai'); //set column field database for datatable searchable 
    ...
    public function get_entries($where = false)
    {
        ...
        if ($where == FALSE) {
            $result = $query->get('tb_absensi as a');
        } else {
            $result = $query->where('a.id', $where)->get('tb_absensi as a');
        }
